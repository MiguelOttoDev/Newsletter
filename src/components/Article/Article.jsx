export default function Article  ({title, text, tags, image, alt})  {
    return(
        <div className="p-5 bg-gray-200 dark:bg-dark-200 sm:rounded-xl gap-4 sm:shadow-lg flex flex-col items-center" >
            ...
            <div className="w-full flex-row justify-end gap-2 pr-5 hidden sm:flex">
                { 
                    tags.map(tag => <span className="bg-alura-100 dark:bg-dark-100 px-4 py-1 rounded-full text-gray-200 dark:text-gray-400 font-bold text-xs uppercase" key={tag}>{tag}</span>)
                }
            </div>
